...
public class Collectable extends Sprite {
    ...
    void createCollectableBody() {
        ...
        bodyDef.position.set((getX() - getWidth() / 2 - 20) / GameInfo.PIXELS_PER_METER,
                (getY() + getWidth() / 2) / GameInfo.PIXELS_PER_METER);

        body = world.createBody(bodyDef);

        PolygonShape shape = new PolygonShape();
        shape.setAsBox((getWidth() / 2) / GameInfo.PIXELS_PER_METER,
                (getHeight() / 2) / GameInfo.PIXELS_PER_METER);
    ...
    public void updateCollectable() {
        setPosition(body.getPosition().x * GameInfo.PIXELS_PER_METER,
                (body.getPosition().y - 0.2f ) * GameInfo.PIXELS_PER_METER);
    }
